...
# --------------------------------------------------------------------------
# 2. 신규 3대 지표 포함 전체 실시간 데이터 자동 크롤링 (1시간 캐싱)
# --------------------------------------------------------------------------
...
def fetch_all_live_data():
    ...
    # 1) ECOS 시장 금리 + 회사채/CP 5년치 실시간 수집 (차트 및 현재치용)
    item_codes = {
    ...
    # 4) [신규 1] FRED 미국 연방기금금리 실시간 수집 (CSV 직접 다운로드)
    try:
    ...
    # 5) [신규 2] ECOS 가계대출 통계 실시간 수집 (151Y001 / 101Y004)
    live_debt_df = pd.DataFrame()
    try:
        # 가계신용 통계표 호출
        url_d = f"https://ecos.bok.or.kr/api/StatisticSearch/{ECOS_API_KEY}/json/kr/1/100/151Y001/Q/20181/20264/1110000"
        res_d = requests.get(url_d, timeout=8).json()
        if isinstance(res_d, dict) and "StatisticSearch" in res_d and "row" in res_d["StatisticSearch"]:
            rows_d = []
            for r in res_d["StatisticSearch"]["row"]:
                t_str = str(r["TIME"])
                rows_d.append({"Date": pd.to_datetime(f"{t_str[:4]}-{int(t_str[4])*3:02d}-01"), "val": float(r["DATA_VALUE"])})
            live_debt_df = pd.DataFrame(rows_d).sort_values("Date").reset_index(drop=True)
            live_debt_df["debt_growth_yoy"] = live_debt_df["val"].pct_change(4) * 100
            live_debt_df = live_debt_df.dropna().reset_index(drop=True)
    except Exception:
        pass

    # 가계신용 응답이 없을 경우 예금은행 대출금 통계 호출
    if live_debt_df.empty:
        try:
            ...
            if isinstance(res_m, dict) and "StatisticSearch" in res_m and "row" in res_m["StatisticSearch"]:
                rows_m = [{"Date": pd.to_datetime(f"{r['TIME'][:4]}-{r['TIME'][4:6]}-01"), "val": float(r["DATA_VALUE"])} 
                          for r in res_m["StatisticSearch"]["row"]]
    ...
    if not live_debt_df.empty and "debt_growth_yoy" in live_debt_df.columns:
        ...
        live_debt_df = pd.DataFrame({
            "Date": m_dates,
            "debt_growth_yoy": [3.2 + 0.05 * i for i in range(len(m_dates))]
        })
...
(auto_msb, auto_ktb, auto_call, auto_corp, auto_cp, auto_net_shift, 
 ...
 mkt_date_str, macro_date_str, live_cred_df, live_us_df, live_debt_df) = fetch_all_live_data()
...
# 스케일러가 요구하는 정확한 피처 목록 자동 매칭 (ValueError 완벽 방어)
...
with tab_us:
    ...
    st.markdown("""
    * **실시간 출처**: 미국 세인트루이스 연준 FRED (`DFF` 실효 연방기금금리)
    ...
    * **역전폭 축소/해소**: 미국 연준의 피벗(인하)으로 한국은행도 비로소 독자적 인하 여력을 확보하게 됩니다.
    ...
    if not live_us_df.empty and not rate_df.empty:
        # 날짜 타입을 표준 datetime으로 변환 후 병합 (MergeError 방어)
...
# ==========================================================================
# [TAB 7] 🏢 회사채 및 CP 신용 스프레드 (실시간 ECOS 5년치 라이브 연동)
# ==========================================================================
with tab_credit:
    st.subheader("🏢 회사채(AA-) 및 단기 자금시장(CP 91일) 신용 스프레드")
    st.markdown("""
    * **실시간 출처**: 한국은행 ECOS 통계표 `722Y001` (시장금리 일별 실시간 호가)
...
# ==========================================================================
# [TAB 8] 🏠 가계대출 증가율 현황 (실시간 ECOS 가계신용 라이브 연동)
# ==========================================================================
with tab_debt:
    st.subheader("🏠 한국은행 예금은행 가계대출 증가율 (YoY %) 추이")
    st.markdown("""
    * **실시간 출처**: 한국은행 ECOS 통계표 `151Y001` (가계신용) 및 `101Y004` (예금은행 가계대출)
